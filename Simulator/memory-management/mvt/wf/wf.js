...
function startColumn2() {
  ...
  var htmlText = `
    <button type="submit" class="btn btn-primary" id="add-pro-btn" style="font-size:1.25rem;">Add process</button>
    <button type="submit" class="btn btn-primary" id="rem-pro-btn" style="font-size:1.25rem;">Remove process</button>
    <a style="text-decoration:none; color:white; font-size:1.25rem;" href="./wf.html"><button type="submit" class="btn btn-primary" id="rem-pro-btn">Reset</button></a>
    `;
...
function addProcess(pro_size, pro_id, fromQ) {
  ...
  if (num_parts == 0 && pro_size <= mem_size) {
    ...
    if (mem_size - part_end[num_parts - 1] >= pro_size) {
      ...
      if (mem_size - part_end[num_parts - 1] > worst_size) {
        worst_ind = num_parts;
        worst_size = mem_size - part_end[num_parts - 1];
        found = 1;
      }
    }
    if (found == 1) {
