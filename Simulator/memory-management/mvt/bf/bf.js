...
function startColumn2() {
  ...
  // // $("#add-rem-pro-btns").css("position", "absolute");
  // $("#add-rem-pro-btns").css("display", "contents");
  // //   $("#column-3").css("position", "relative");
  var htmlText = `
  <button type="submit" class="btn btn-primary" id="add-pro-btn" style="font-size:1.26rem;">Add process</button>
  <button type="submit" class="btn btn-primary" id="rem-pro-btn" style="font-size:1.26rem;">Remove process</button>
...
function addProcess(pro_size, pro_id, fromQ) {
  ...
  var found = 0;
  var best_ind = -1;
  var best_size = mem_size;
  if (num_parts == 0 && pro_size <= mem_size) {
    best_ind = 0;
    best_size = mem_size;
    ...
    if (mem_size - part_end[num_parts - 1] >= pro_size) {
      console.log(best_size);
      if (mem_size - part_end[num_parts - 1] < best_size) {
        best_ind = num_parts;
        best_size = mem_size - part_end[num_parts - 1];
        found = 1;
      }
    }
    if (found == 1) {
...
function drawPart() {
  ...
  for (i = 0; i < num_parts; i++) {
    ...
    ctx.rect(
      myCanvas_x_start,
      myCanvas_y_start + part_start[i] * (500 / mem_size),
      myCanvas_width,
    ...
    ctx.fillStyle = "black";
    ctx.fillText(
      "P-" + String(part_pro_id[i]) + ", size: " + String(part_size[i]),
      50,
      myCanvas_y_start +
        part_start[i] * (500 / mem_size) +
        (part_size[i] * (500 / mem_size)) / 2
    );
  }
}
...
function calcExtFrag(pro_size) {
  ...
  tot_hole_size += mem_size - part_end[num_parts - 1];
